...
with st.echo(code_location='below'):
    ...
    st.write('1) Pandas was used almost everywhere, it is not difficult to see from the code below.')
    st.write('2) The Selenium library was used to load the SQL table. Ilya said that ipnb with Selenium should be attached separately, because it does not work together with Streamlite. You can look at the code in the zip archive attached to the homework.')
    st.write('3) REST API was used in the analysis of the popularity of a selected celebrity depending on the country income. Data on income levels in different countries are downloaded from the World Bank website.')
    st.write('4) The graphs were constructed using the plotly and matplotlib libraries.')
    st.write('5) The numpy library was used in the statistical block to calculate correlations, expected value, and variance.')
    st.write('6) Streamlit was obviously used.')
    st.write('7) SQL was used at the very end in the creation of the new popularity index.')
    st.write('8) Regular expressions were not used when the project was created.')
    st.write('9) Geopandas was used in the construction of the chart with the map')
    st.write('10) Machine learning: sklearn (a linear regression model) was used to predict the popularity index.')
    st.write('11) Graphs were not used when the project was created.')
